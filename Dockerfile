...
RUN last_version=`curl https://github.com/miniflux/miniflux/tags 2>/dev/null | grep "miniflux/releases/tag" | head -n 1 | sed 's#.*tag/\([^"]*\).*#\1#'`
RUN cd miniflux && make miniflux VERSION=$last_version && make clean && mv miniflux /usr/local/bin
RUN apk del git
